...
interface IModelField {
    type: 'text' | 'checkbox' | 'textarea' | 'select' |  'number' | 'password' | 'date' | 'time' | 'datetime' | 'email';
    name: string;
    readonly?: boolean;
    required?: boolean;
}

interface IModelFieldsGroup {
    [key: string]: {
        'fields': string[];
        'title'?: string;
        'description'?: string;
...
interface IModelForm {
    ...
    group?: IModelFieldsGroup;
}

function ModelField({type, name, readonly, required}: IModelField) {
    let fieldItem = null;
    
    if (type == 'text' || type == 'number' || type == 'password' || type == 'email') {
        fieldItem = <Input id={name} type={type} label={name} disabled={readonly} required={required} />
    }
    else if (type == 'checkbox') {
        fieldItem = <Checkbox id={name} label={name} disabled={readonly} required={required} />
    }
...
function ModelFieldsFetch() {}

function ModelForm({modelName, modelFields, group}: IModelForm) {
    return (
